...
import { clearGeminiSettings, getGeminiSettings, saveGeminiSettings, testGeminiSettings } from '../services/geminiSettings.js?v=111';
...
export function renderSettingsView(container, onNavigate) {
  ...
  getSpeechAvailability('lt-LT').then(availability => {
    ...
    status.textContent = availability.source === 'device' ? `Lithuanian audio is ready with ${availability.voiceName || 'the installed device voice'}${availability.voiceLocale ? ` (${availability.voiceLocale})` : ''}.`
      : availability.source === 'android' ? 'Lithuanian audio will use Android text-to-speech.'
        : availability.source === 'gemini' ? `Gemini ${gemini.ttsVoice} is configured. Use Test Lithuanian audio to verify the saved key.`
  ...
  container.querySelector('#test-lithuanian-audio').addEventListener('click', async event => {
    ...
    try {
      ...
      if (status) status.textContent = played ? 'Lithuanian audio is working.' : getLastSpeechErrorCode() === 'invalid-gemini-key' ? 'The saved Gemini key was rejected. Replace it with a valid Google AI Studio key.' : 'Audio could not start. Check the API key or install an lt-LT voice on this device.';
    } catch (error) {
